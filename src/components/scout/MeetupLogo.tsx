...
const PALETTES: Record<MeetupGroup["palette"], { bg: string; ring: string; fg: string; mark: string }> = {
...
export function MeetupLogo({ group, size = "md" }: { group: MeetupGroup; size?: "md" | "lg" }) {
  ...
  return (
    <div className={cn("relative grid place-items-center rounded-full ring-1", dim, p.bg, p.ring)}>
      <Icon className={cn(iconSize, p.fg)} strokeWidth={1.75} />
      <span
        className={cn(
          "absolute -bottom-1 -right-1 grid place-items-center rounded-full font-display font-bold shadow-sm",
          badgeSize,
          p.mark,
        )}
      >
